...
# ---- sudoers entry: 让 admin 用户能 kickstart 这些 daemon 不要密码 ----
# 用途: socket_vmnet daemon 长跑后 (实测 ~1.5h+) 在 bridged 模式下进入 stale 状态 —
# 能 accept QEMU connect 但 vmnet kext frame 转发僵死. 重启 daemon (launchctl
# kickstart -k) 立即恢复. HVM GUI 启动时自动 kickstart 所有 hvm vmnet daemon
# 让它们 fresh, 避免 user 抱怨 "突然 bridged 没网". 没这个 sudoers 就要密码,
# GUI 没法弹 prompt, 自动恢复机制断链.
# 安全: 限制只能 kickstart hvm vmnet daemon (两个 label 前缀都允许:
# 现行 com.hellmessage.hvm.vmnet.* 跟旧版 io.hell.vmnet.* — 跟 hell-vm 同款),
# 不能 kickstart 任意 launchd service.
SUDOERS_FILE="/etc/sudoers.d/hvm-vmnet-kickstart"
info "装 sudoers entry: $SUDOERS_FILE (让 admin 不要密码 kickstart hvm vmnet daemon)"
cat > "$SUDOERS_FILE" <<EOF
# Generated by HVM install-vmnet-helper.sh - DO NOT EDIT MANUALLY
# Allows admin users to kickstart HVM / hell-vm vmnet daemons without password.
# HVM GUI uses this to refresh stale socket_vmnet sessions on startup, avoiding
# the "bridged network suddenly stops working after ~1.5h" issue.
# Scope: only ${LABEL_PREFIX}.* (current) and io.hell.vmnet.* (legacy / hell-vm).
%admin ALL=(root) NOPASSWD: /bin/launchctl kickstart -k system/${LABEL_PREFIX}.*
%admin ALL=(root) NOPASSWD: /bin/launchctl kickstart -k system/io.hell.vmnet.*
EOF
chmod 0440 "$SUDOERS_FILE"
chown root:wheel "$SUDOERS_FILE"
# visudo -c 验证 syntax (失败立即删, 防止整个 sudoers 损坏)
if ! visudo -c -f "$SUDOERS_FILE" >/dev/null 2>&1; then
    rm -f "$SUDOERS_FILE"
    warn "sudoers entry 语法校验失败, 已删除 (HVM GUI auto-refresh 不可用, 不影响其他功能)"
fi
