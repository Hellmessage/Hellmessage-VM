...
//
// PPM P6 文件结构: "P6\n" magic / 可选 # 注释行 / "<w> <h>\n" / "<maxval>\n" / 二进制 RGB.
// 仅解码 maxval=255 (8-bit RGB); 16-bit 不支持.
...
public enum PPMReader {
    ...
    /// 只读 PPM 头拿 width/height, 不解 binary pixel. 用于 hvm-dbg display-info
    /// 比对 resize 前后尺寸变化, 不必传完整 PNG.
    ...
    public static func decode(_ data: Data) throws -> CGImage {
        ...
        // 取 ceil(w/2)*2, 每行 stride = physical_w*3; 但 PPM header 写逻辑 width, 违反 PPM spec
        // (stride = w*3 no padding), 不修正会让 screenshot 图片倾斜.
        // 用 (body_size / height) 推 actual stride, 每行只取前 width*3 字节重组 packed buffer.
