...
//
// scrollerStyle = .overlay (浮动式, 永不预留空间) + hasVerticalScroller = false; 监听
// NSScrollView frame 变化, resize 时重新应用 (防 SwiftUI 重布局把 style 改回 legacy 又预留空间).
// 用法: ScrollView { content.hvmHideScroller() }
...
private struct ScrollerHider: NSViewRepresentable {
    ...
    // 只在主线程用; @unchecked Sendable 让捕进 DispatchQueue 闭包不报数据竞争, NSObject 供 @objc selector.
    final class Coordinator: NSObject, @unchecked Sendable {
        private weak var scrollView: NSScrollView?

        func attach(to v: NSView) {
            DispatchQueue.main.async { [weak self] in
                // main 队列即主线程, assumeIsolated 让下方访问 main-actor NSView 属性合法
                MainActor.assumeIsolated {
                    guard let self else { return }
                    guard let sv = self.findScrollView(from: v) else {
                        // NSScrollView 可能还没建好, 短延迟重试
