...
- **跑 Linux arm64 guest** — 通过 ISO 半自动安装 (Ubuntu / Debian / Fedora arm64 等), 装完切 `bootFromDiskOnly` 走硬盘启动. 默认 VZ 后端, 高级模式可选 QEMU
- **跑 macOS guest** — 通过 IPSW 全自动装机, 内建从 Apple CDN 拉最新 IPSW(支持断点续传). 仅 VZ 后端
- **跑 Windows arm64 guest** (实验性) — Win11 走 **QEMU + HVF** + 包内 swtpm (TPM 2.0) + 包内 EDK2 (SecureBoot); virtio-win 驱动 ISO 自动下载缓存
- **GUI 全鼠标操作** — 黑色主题主窗口、创建向导(Linux/macOS/Windows 三选一)、独立/嵌入运行窗口、菜单栏 popover、缩略图列表
...
- **AI agent 友好** — `hvm-dbg` 提供 screenshot / status / key / mouse / console / exec 等子命令, 不依赖 osascript 也能操控 guest (VZ 后端; QEMU 后端 dbg.* 暂未实现)
- **零三方依赖 (运行时)** — 最终用户机器**不需要** Homebrew, 仅 Xcode CLT 即可. QEMU + swtpm + 全部依赖 dylib 随 `.app` 分发, 由 `make build-all` 一次打包
...
- ❌ **热插拔 CPU / 内存** — 改配置必须停机
- ❌ **QEMU 显示嵌入主窗口** — QEMU 走 `-display cocoa` 自开独立 NSWindow, 不嵌入 HVM 主窗口右栏 (待评估 SPICE 方案)
...
```bash
make qemu               # 首次 10-30 分钟: 自动装 brew 依赖 + 拉 v10.2.0 源码 + 编译 + 嵌入
                        # 产物落 third_party/qemu/{bin,lib,share}, 含 swtpm + 全部 dylib (重定向)
make build-all          # = make qemu + make build (.app 自带 QEMU + swtpm + EDK2, 共 ~66MB)
```

跑 `make qemu` 需要联网 + 几 GB 临时编译空间. 一次跑完后 `third_party/qemu/` 缓存在仓库内 (gitignored), 后续只需 `make build` 重新打包 (不重编译 QEMU)。
