...
- **CLI 自动化** — `hvm-cli` 子命令覆盖 list / create / start / stop / pause / resume / config / disk / snapshot / iso / boot-from-disk / install / logs / ipsw / osimage;`--engine vz|qemu` 走 ArgumentParser 自动校验
...
- **GPL 合规** — QEMU 上游 commit SHA + tag + license 全文写入 `Resources/QEMU/MANIFEST.json` 与 `Resources/QEMU/LICENSE`
...
make qemu               # 首次 10-30 分钟: 装 brew 依赖 + 拉 v10.2.0 源码 + 编译 + 嵌 swtpm
                        # 源码落 third_party/qemu-src/    (~900M, gitignored)
                        # 产物落 third_party/qemu-stage/  (~180M, 含 swtpm + dylib 重定向;
                        #                                  socket_vmnet 不入包, 由用户 brew install)
...
```bash
make dev                # debug 模式
make verify             # smoke test, 验证 .app 可启动 + 签名 + entitlement + patches 孤儿检测 + GUI 约束 (无 NSAlert) + 可启动
...
# QEMU 自开 cocoa 窗口, Win11 安装界面出现; 装机时驱动自动加载
# 装完关机后:
...
详见 [docs/v1/DEBUG_PROBE.md](docs/v1/DEBUG_PROBE.md)。
...
│   └── u1.hvmz/
│       ├── config.yaml        # bundle 元数据 (YAML 1.1, schema v2)
...
| 产物 | 角色 |
|---|---|
| `HVM.app` | GUI 主入口,同时也是 VM host 进程(`--host-mode-bundle` 起 `HVMHost`) |
| `hvm-cli` | 短命 CLI,操作 bundle 或对已有 host 发 IPC,不常驻 |
| `hvm-dbg` | 调试探针,给 AI agent / 自动化测试用;零新协议,只复用公开 VZ API |

源码模块拓扑见 [docs/v1/ARCHITECTURE.md](docs/v1/ARCHITECTURE.md)。
...
- **[docs/v1/](docs/v1/)** — 现状描述(按当前代码逻辑重构,2026-05-04 全量更新)
- **[docs/v2/](docs/v2/)** — 优化与技术债 TODO(全项目深审产出,大部分已完成)

推荐阅读顺序:

1. [docs/v1/ARCHITECTURE.md](docs/v1/ARCHITECTURE.md) — 项目全貌、模块划分、进程模型
2. [docs/v1/ROADMAP.md](docs/v1/ROADMAP.md) — 里程碑与不做清单
3. [docs/v1/VM_BUNDLE.md](docs/v1/VM_BUNDLE.md) — `.hvmz` 目录布局与 `config.yaml` schema(YAML 1.1, v2)
