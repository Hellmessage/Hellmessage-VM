...
| **M3** | macOS guest (IPSW + `VZMacOSInstaller`) | 🚧 |
...
- **跑 macOS guest** (M3 进行中) — 通过 IPSW 全自动装机
- **GUI 全鼠标操作** — 黑色主题主窗口、创建向导、独立/嵌入运行窗口、菜单栏 popover、缩略图列表
- **CLI 自动化** — `hvm-cli` 子命令覆盖 list / create / start / stop / pause / resume / config / disk / snapshot / iso / boot-from-disk / install / logs
- **AI agent 友好** — `hvm-dbg` 提供 screenshot / status / key / mouse / console / exec 等子命令, 不依赖 osascript 也能操控 guest
- **零三方依赖** — 仅 Apple framework + `swift-argument-parser`, 一条 `make build` 出带签名的 .app

## 不做什么(能力边界)

VZ framework 的硬限制, 即使有需求也不会实现:

- ❌ **Windows guest** — VZ 无 TPM, Win11 装不了; Win10 ARM 已停供 ISO
...
4. 其他专题 (CLI / GUI / NETWORK / STORAGE / GUEST_OS_INSTALL / ...) 按需读
