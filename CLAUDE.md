...
- **所有破坏性操作必须二次确认** (硬约束): 删除 VM / 删除磁盘 / 删除网卡 / 删除共享目录 / 强制停止(kill) / 解密 / secure-erase / 任何不可逆或丢数据的动作, 点击后必须先弹 `dialog.confirm(destructive: true)` (老 GUI 走 ConfirmDialog), 用户确认才执行. **禁止**破坏性按钮直接触发动作. 新增任何"删除/清除/强制/重置/覆盖"类操作时一并加确认
- 主窗口默认深色, 不跟随系统主题
