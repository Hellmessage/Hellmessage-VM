...
- **不带文件**: `.lock` (目标首次启动自然创建) / `logs/console-*.log` / `.unattend-stage` + `unattend.iso` (Win 装机产物按需重生) / `snapshots/` (默认; `--include-snapshots` 可带)
...
- **加密 VM 克隆**: 走 `docs/v3/ENCRYPTION.md` 合入后再补 wrapper (sparsebundle 复制 + chpass), 不在 CloneManager 范围
...
- **设计稿位置**: `docs/v3/<TOPIC>.md` (单提案单文档, 状态机: `设计稿` → `评审中` → `实现中` → `代码已合入`); 索引登 `docs/v3/README.md`
- **设计稿必须包含**:
  - **目标 + 范围** (做什么, 不做什么)
