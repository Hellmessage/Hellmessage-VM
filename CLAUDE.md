...
- **构建参数固定**: QEMU `--target-list=aarch64-softmmu --enable-cocoa --enable-hvf -Diosurface=enabled`; EDK2 `-p ArmVirtPkg/ArmVirtQemu.dsc -a AARCH64 -t GCC5 -b RELEASE` (cross compile via brew aarch64-elf-gcc), 控体积与签名面. `-Diosurface=enabled` 是 HVM patch 0002 引入的 macOS-only display backend (`-display iosurface,socket=...`, AF_UNIX + POSIX shm + SCM_RIGHTS), 协议规范见 `docs/QEMU_DISPLAY_PROTOCOL.md` v1.0.0; **必须用 `-D` meson 直传形式** — QEMU configure 包装层只识别 hardcoded `--enable-XX` 列表, 自家 patch 引入的 feature 走 meson 直传绕开 configure 校验. cocoa 保留作 fallback/调试, 生产路径由 HVM 主进程 argv 选 `iosurface`
...
- **依赖配套**:
  ...
  - `socket_vmnet` 由 brew 锁版本 (QEMU 走 vmnet bridged/shared 网络的非 root 桥接), 同模式打包入包内. `socket_vmnet_client` wrapper 也一并打包(仅作排查兜底, 运行时不引用; 见下条网络约束)
...
- **socket_vmnet 网络约束**: macOS `vmnet` 必须 root, QEMU 走 socket_vmnet 跨 vmnet 的方案要求一次性 sudoers NOPASSWD 配置:
  ...
  - **QEMU 接 socket_vmnet 协议**: socket_vmnet daemon 走 4-byte length-prefix framing(私有协议), 与 QEMU `-netdev stream`(裸字节流)**不兼容**, 也**不能用** `socket_vmnet_client` wrapper(单 fd 限制, 多 NIC 装不下). HVM/hvm-dbg 父进程自己 `socket(AF_UNIX)+connect()` 每个 daemon socket, 通过 `posix_spawn_file_actions_adddup2` 把 N 个 fd 落到 QEMU 子进程的 `fd 3, 4, 5...`, QEMU argv 写 `-netdev socket,id=netN,fd=K`(与 lima `pkg/driver/qemu/qemu_driver.go` `fd_connect` 等价). 实现位于 `app/Sources/HVMQemu/SidecarProcessRunner.swift` 的 `startWithPosixSpawn()`. 普通用户启动 / 关闭 VM 完全免密
  - **多 NIC 桥接**: 同一 VM 可挂多个 bridged/shared NIC, 顺序对应 `BuildResult.vmnetSocketPaths` 列表, fd 编号按数组顺序从 3 起递增. NAT NIC 不占 fd
