...
- **socket_vmnet 网络约束**: macOS `vmnet` 必须 root, QEMU 走 socket_vmnet 跨 vmnet 的方案要求一次性 sudoers NOPASSWD 配置:
  - 用户首次创建 bridged/shared QEMU VM 时由 `scripts/install-vmnet-helper.sh` 写 `/etc/sudoers.d/hvm-socket-vmnet` (一次性), 之后 sidecar 启动无密码
  - 每个 QEMU bundle 启动时 fork 一个 per-VM `socket_vmnet` 进程, 监听 `~/Library/Application Support/HVM/run/<vm-id>.vmnet.sock`
  - QEMU 通过 `-netdev stream,addr.type=unix,addr.path=...` 接入, 不走老的 `socket_vmnet_client` fd-passing 路径
  - **bridged 接口固定走 `vmnet-bridged`**(物理 LAN 桥接), 不允许在用户 sudoers 范围之外注入额外参数 (防止提权)
