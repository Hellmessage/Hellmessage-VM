...
## [x] #1 · `bundle.sh` 给 dylib/libexec 签名加了 `|| true`,会让用户启动 .app 时被 AMFI 拒签 — 已修 (commit 29386eb)
...
## [x] #2 · `install-vmnet-daemons.sh` 白名单 reject 后只 `continue`,脚本仍 exit 0 — 已修 (commit dffd332)
...
## [x] #3 · swtpm sidecar 进程清理时序错,可能写坏 NVRAM — 已修 (commit 35a3954)
...
## [x] #4 · App 退出时 graceful shutdown 不等 forceStop 完成,留孤儿 QEMU/swtpm — 已修 (commit 35a3954)
...
## [x] #5 · `BundleLock.release()` 非线程安全,double-close 风险 — 已修 (commit dffd332)
...
## [x] #6 · `qemu-build.sh` 的 `mktemp` 没 trap,异常退出泄露 tmp — 已修 (commit dffd332)
...
- [x] 所有 6 项代码已修(commits dffd332 / 29386eb / 35a3954)
- [x] `make build` + `make install` 跑通
- [ ] **待端到端实测**: Win11 + Linux + macOS 装机闭环, 退出时无孤儿进程(留给用户跑一次, 任何 regression 回填到本文档"已知问题"段)
- [ ] **待文档同步**: CLAUDE.md / docs/v1/ 影响约束的章节(主要是 BUILD_SIGN.md 与 NETWORK.md)— 可批量做, 不阻塞 P1

## 修复 commit 索引

| 项 | 内容 | commit |
|---|---|---|
| #5 | BundleLock NSLock 防 double-close | [`dffd332`](https://example/commit/dffd332) |
| #6 | qemu-build.sh mktemp 套 sub-shell + EXIT trap | [`dffd332`](https://example/commit/dffd332) |
| #2 | install-vmnet-daemons.sh 接口名白名单 fail-fast | [`dffd332`](https://example/commit/dffd332) |
| #1 | bundle.sh dylib/libexec 严格签名 + process substitution | [`29386eb`](https://example/commit/29386eb) |
| #3 | SidecarProcessRunner 双段杀 + reap 等待 | [`35a3954`](https://example/commit/35a3954) |
| #4 | HVMApp.gracefulShutdownAll 等真退 + 错误日志 | [`35a3954`](https://example/commit/35a3954) |
