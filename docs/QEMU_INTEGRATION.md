...
| M4 | ✅ **已决（hell-vm 风格 socket_vmnet）**: 5-mode 网络栈 (user / vmnetShared / vmnetHost / vmnetBridged / none) 跨 VZ + QEMU. QEMU 后端走系统级 socket_vmnet launchd daemon (`brew install socket_vmnet` + `scripts/install-vmnet-daemons.sh`). 提权用 osascript admin Touch ID, 不写 sudoers. 详见 [NETWORK.md](NETWORK.md). |
...
5-mode 网络栈跨 VZ + QEMU 两个后端 (详见 [`NETWORK.md`](NETWORK.md)):

| `mode` | QEMU 后端 | VZ 后端 |
|---|---|---|
| `user` | `-netdev user` (SLIRP) | `VZNATNetworkDeviceAttachment` |
| `vmnetShared` | `socket_vmnet --vmnet-mode=shared` | 退化 NAT |
| `vmnetHost` | `socket_vmnet --vmnet-mode=host` | 退化 NAT |
| `vmnetBridged` | `socket_vmnet --vmnet-mode=bridged --vmnet-interface=<iface>` | `VZBridgedNetworkDeviceAttachment` (依赖 Apple entitlement) |
| `none` | NIC 跳过 | NIC 跳过 |

socket_vmnet 是 macOS `vmnet` 的非 root 桥接 daemon (Lima 项目). HVM 走 hell-vm 风格集成:

- **socket_vmnet 不打包入 .app**: 用户机器 `brew install socket_vmnet` 装二进制, `scripts/install-vmnet-daemons.sh` 从 brew 路径 (`/opt/homebrew/opt/socket_vmnet/bin/socket_vmnet`) 拉 binary 写 launchd plist
- **系统级 daemon, 跨 VM 共享**: 单 daemon 跑 root, 所有 VM 都连同一个固定 socket. plist 路径 `/Library/LaunchDaemons/com.hellmessage.hvm.vmnet.<mode>.plist`, KeepAlive 由 launchd 管
- **socket 路径** (跟 socket_vmnet 上游 / lima / hell-vm 一致):
  - `/var/run/socket_vmnet`              (shared)
  - `/var/run/socket_vmnet.host`         (host)
  - `/var/run/socket_vmnet.bridged.<iface>` (bridged)
- **QEMU 直连**: `-netdev stream,id=netN,addr.type=unix,addr.path=<sock>`. socket_vmnet 4-byte length-prefix framing 与 QEMU stream 协议一致 (lima / hell-vm 同款), 无需 socket_vmnet_client wrapper, 无需父进程 fd 透传
- **提权**: GUI `编辑配置 → 网络 → 安装 daemon` 通过 `VMnetSupervisor.installAllDaemons` 走 `osascript "do shell script ... with administrator privileges"` 弹原生 Touch ID / 密码框, 一次到位装 shared + host + N 个 bridged.<iface>. 不写 sudoers, 不拉 Terminal, 不自动 kickstart

### 用户首次配置

```bash
# 1. 装 socket_vmnet 二进制 (brew 锁版本)
brew install socket_vmnet

# 2a. GUI 路径: HVM → 编辑配置 → 网络面板 → "安装 / 更新 daemon" (Touch ID)
# 2b. CLI 路径 (可选, 等价于 GUI 按钮):
sudo scripts/install-vmnet-daemons.sh                # shared + host
sudo scripts/install-vmnet-daemons.sh en0 en1        # + bridged(en0) + bridged(en1)
```

之后所有 VM 启动 / 关闭都不再需要密码 (daemon 由 launchd 拉起, QEMU 普通用户身份连 unix socket).

### 卸载

```bash
sudo scripts/install-vmnet-daemons.sh --uninstall
# 或 GUI: 编辑配置 → 网络面板 → "卸载全部"
```

**移动 .app 后**不需要重跑 install (plist 内 `ProgramArguments[0]` 是 brew 绝对路径, 与 .app 位置无关; install-vmnet-daemons.sh 本身只在 GUI 拉脚本时按 Bundle.main 定位).
