...
- `Resources/QEMU/bin/*` + `Resources/QEMU/lib/*.dylib` + `Resources/QEMU/libexec/*` 全部**逐文件** codesign, 严格不吞错 (老版本 `|| true` 软模式已修)
...
- `hvm-dbg` 全部子命令 (screenshot / key / mouse / ocr / find-text / wait / boot-progress / console / exec / exec-guest / display-info / display-resize) 已双后端通路, 客户端无感知差异; `exec-guest` 仅 QEMU (走 qemu-guest-agent JSON-RPC)
