...
hvm-cli create ...                   创建新 VM bundle (linux / macOS / windows; 含 --encrypt)
hvm-cli delete <vm> [--purge]        删除 VM
hvm-cli clone <vm> --name <new>      整 VM 克隆 (APFS clonefile + 身份字段重生; 加密 VM 同密码)
hvm-cli start <vm>                   启动 VM (拉起 HVMHost 子进程; 加密 VM 走 prompt 或 --password-stdin)
...
hvm-cli disk <vm> list|add|resize|delete   磁盘操作 (vz=raw / qemu=qcow2 / qemu加密=LUKS qcow2)
...
hvm-cli encrypt <vm>                 明文 → 加密 VM (冷迁移; 仅 QEMU)
hvm-cli decrypt <vm>                 加密 → 明文 VM (冷迁移; 仅 QEMU)
hvm-cli rekey <vm>                   改密 (LUKS keyslot 重写; 仅 QEMU)
hvm-cli encrypt-status <vm>          看加密形态 + KDF 参数 (走 routing JSON, 不需密码)
```
...
| `--encrypt` | 创建加密 VM (强制 engine=qemu, prompt 密码 + 二次确认) |

### `start <vm>`

```
1. 检查 <bundle>/.lock fcntl flock, 已被锁 → 报 bundle.busy (退出 4)
2. 加密 VM: 走 EncryptedBundleIO.detectScheme + tty prompt 密码 (或 --password-stdin 从管道读)
3. HostLauncher.launch(bundleURL:): fork HVM.app 自身 + argv `--host-mode-bundle <path>`
4. VMHost 启动后写 .lock + IPC socket; CLI 立即返回
```

stdout/stderr 走全局 log: `~/Library/Application Support/HVM/logs/<displayName>-<uuid8>/host-<date>.log` (CLAUDE.md 日志路径约束)。

加密 VM 启动: `--password-stdin` 从 stdin 读密码 (避免 history 泄露 / 自动化场景), 否则 tty prompt; VZ sparsebundle 加密 VM 当前直接拒启 (路径推后)。
...
字段重生 / 保留矩阵详见 [VM_BUNDLE.md](VM_BUNDLE.md) 顶层字段表 + [STORAGE.md "Clone"](STORAGE.md) + [CLONE.md](CLONE.md).

加密源 VM clone 走"等价复制 + 同密码"分支 (D9), `prompt` 密码后字节级 COW 复制 LUKS qcow2 + nvram + tpm + 重新加密 config.yaml.enc; 想换密码自跑 `hvm-cli rekey`.

### `encrypt <vm>` / `decrypt <vm>` / `rekey <vm>` / `encrypt-status <vm>`

整 VM 加密 (cold migration, 仅 QEMU per-file scheme; VZ sparsebundle 路径暂只接通"创建", 启动解锁推后):

```
hvm-cli encrypt <vm>           明文 → 加密. VM 必 stopped, 走 prompt 输密码 + 二次确认
                               Win VM 重置 TPM (swtpm key 重生, BitLocker 失效)
hvm-cli decrypt <vm>           加密 → 明文. 走 prompt 输原密码; disks 仍 qcow2 (LUKS 拆封)
hvm-cli rekey <vm>             改密. prompt 原密码 + 新密码; LUKS keyslot 毫秒级
                               Win VM 重置 TPM
hvm-cli encrypt-status <vm>    打印 scheme / kdf_algo / kdf_iterations / kdf_salt(base64) /
                               encrypted_paths. 走 routing JSON 不需密码
```

`--force` 跳过二次确认; 加密事务底层走 `EncryptVMOperation / DecryptVMOperation / RekeyVMOperation`, 长事务 SIGINT 防中断 (`SignalGuard` 由 v3 SIGINT_CLEANUP.md 提供). 详见 [ENCRYPTION.md](ENCRYPTION.md).
...
- [DEBUG_PROBE.md](DEBUG_PROBE.md) — `hvm-dbg`, guest + GUI 内自动化
- [GUEST_OS_INSTALL.md](GUEST_OS_INSTALL.md) — `install` / `ipsw` / `osimage` 背后流程
- [VM_BUNDLE.md](VM_BUNDLE.md) — config.yaml schema v3 字段语义
- [ENCRYPTION.md](ENCRYPTION.md) — `encrypt` / `decrypt` / `rekey` / `encrypt-status` 背后流程
- [CLONE.md](CLONE.md) — `clone` 背后流程
